/** @file gsErrEstFvwSpaceTimeMajorantII.h
...
*/
    template <class T>
    class gsErrEstFvwSpaceTimeMajorantII : public gsEstimator<T>
    {
    ...
    public:
        ...
        */
        gsErrEstFvwSpaceTimeMajorantII(const gsField<T> & _approxV,
        ...
        */
                         ...
                         T & total)
        {
            ...
            for (index_t k = 0; k < quWeights.size(); ++k) // loop over quadrature nodes
            {
                ...
                // F_K(v, w) := {grad_x v * grad_x (w - v) + v_t * (w - v) - f * (w - v)}_K
                incrSq += weight * ( m_phApproxSpaceGradV.col(0).transpose() * (m_phApproxSpaceGradW.col(0) - m_phApproxSpaceGradV.col(0))
                                     + (m_phApproxTimeGradV(0, 0) - m_rhsVals(0, k)) * (m_approxW(0, k) - m_approxV(0, k)));
            }
            total += incrSq;
            return incrSq;
        }

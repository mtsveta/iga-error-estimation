...
*
* The error indicator \f$\overline{\rm m}^2_d\f$ for a computed discrete solution \f$ v \f$ is given by
* \f[ \overline{\rm m}^2_d := \sum_K \overline{\rm m}^2_{d, K} \f]
* where the local estimate \f$ \overline{\rm m}^2_{d, K} \f$ on an element \f$ K \f$ is given by
 \f[
 \overline{\rm m}^2_{d, K} := \int_K ( \grad v - y )^2 dx
 \f]
...
*/
    template <class T>
    class gsErrEstDualMajorant : public gsNorm<T>
    {
    ...
    public:
        ...
        */
        ...
        gsErrEstDualMajorant(const gsField<T> & _approxSol,
    ...
    protected:
        ...
        */
        ...
        inline T compute(gsDomainIterator<T>    & element,
            ...
            gsMatrix<T> m_phApproxSolGrad;    // grad(u_h)
            m_phApproxSolGrad.setZero(m_approxSolGrad.rows(), 1);
            ...
            for (index_t k = 0; k < quWeights.size(); ++k) // loop over quadrature nodes
            {
                ...
                // |grad v - y|^2 := (v_{x1} - y_1)^2 + (v_{x2} - y_2)^2
                mdSq += weight * (m_phApproxSolGrad - m_approxFluxVals.col(k)).squaredNorm();
            }
            maj += mdSq;
    ...
    private:
        ...
        //gsMatrix<T> m_phApproxFluxVals;   // y_h
        //gsMatrix<T> m_phApproxSolGrad;    // grad(u_h)

        int m_parDim;              // dimension of the parameter domain
